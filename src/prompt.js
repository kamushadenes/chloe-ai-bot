import {setMode, getMode} from "./db.js";

const modes = {
    "default": defaultMode,
    "act": act,
    "linuxTerminal": linuxTerminal,
    "englishTranslatorAndImprover": englishTranslatorAndImprover,
    "interviewer": interviewer,
    "storyTeller": storyTeller,
    "standUpComedian": standUpComedian,
    "motivationCoach": motivationCoach,
    "debater": debater,
    "movieCritic": movieCritic,
    "cyberSecuritySpecialist": cyberSecuritySpecialist,
    "recruiter": recruiter,
    "promptGenerator": promptGenerator,
    "midjourneyPromptGenerator": midjourneyPromptGenerator,
    "lunatic": lunatic,
...
async function act(env, userId) {
    let result = await getMode(env, userId);

    let character = result?.actCharacter || "Chloe";
    let series = await result?.actSeries || "Detroit: Become Human";
...
async function interviewer(env, userId) {
    ...
    let position = result?.interviewerPosition || "Software Engineer";
...
export async function getSystemPrompt(env, message) {
    let result = await getMode(env, message.from.id);
    let mode = result?.mode || "default";
